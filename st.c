...
static void tcreateimgplaceholder(uint32_t, uint32_t, int, int, char);
...
ttyread(void)
{
	...
	default:
		...
		while (1) {
			...
			// buf is limited, and we don't clean it in this
	...
	while (n > 0) {
		...
		if (FD_ISSET(cmdfd, &wfd)) {
			...
			if (r < n) {
				 ...
				 * This means the buf is getting full
				 * again. Empty it.
				 */
				if (n < lim)
					lim = ttyread();
	...
	for (lim = args + narg; args < lim; ++args) {
		if (priv) {
			...
			case 1049: /* swap screen & set/restore cursor as xterm */
				...
				/* FALLTHROUGH */
			case 47: /* swap screen buf */
			case 1047: /* swap screen buf */
				if (!allowaltscreen)
					break;
				if (set)
	...
	case 't': /* title stack operations ; XTWINOPS */
		...
		case 16: /* character cell size in pixels */
			n = snprintf(buf, sizeof buf, "\033[6;%d;%dt",
			             term.pixh / term.row, term.pixw / term.col);
			ttywrite(buf, n, 1);
			break;
