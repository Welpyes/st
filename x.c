...
xdrawundercurl(Draw draw, Color *color, int x, int y, int w, int h, int thick)
{
	...
	for (int i = x - (x % wavelen); i < x + w; i += wavelen) {
		XPoint points[3] = {{.x = (short)i, .y = (short)(y + yoffset)},
				    {.x = (short)(i + segw), .y = (short)(y + yoffset + segh)},
				    {.x = (short)(i + wavelen), .y = (short)(y + yoffset)}};
		...
		FD_SET(xfd, &rfd);
				...
				if (graphics_next_redraw_delay != INT_MAX &&
					...
					timeout = timeout < 0 ? graphics_next_redraw_delay
		                ...
		                seltv.tv_sec = timeout / 1E3;		seltv.tv_nsec = 1E6 * (timeout - 1E3 * seltv.tv_sec);
